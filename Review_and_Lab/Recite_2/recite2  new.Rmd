...
### Definition
- The r.v.s are called a random sample of size n from the population f(x) if $X_1, ..., X_n$ are mutually independent and have the same p.d.f/p.m.f f(x). Alternatively, $X_1, ..., X_n$ are called independent,and identically distributed random variable with p.d.f/p.m.f ,commonly abbreviated to i.i.d. r.v.s.
...
### Definition
- $X_1,...,X_n$ is a random sample of size n from the population f(x). A statistic is a real-valued or vector-valued function fully depended on $X_1, ..., X_n$, thus $$T = T(X_1,...,X_n)$$
...
- Let $X_1,...,X_n$ be an random variables or sequence, is said to converge in probability to a value b if for every  $\varepsilon > 0$,$$P(\|X_n-b\|>\varepsilon)\rightarrow 0$$
as $n\rightarrow \infty$ We write this $X_n \mathop{\longrightarrow}^{p} b$ or $plim(X_n)=b$
...
- Intuition:the distribution of ??collapses?? $\bar{X}$ on $\mu$
...
### Example
- Suppose X has a Bernoulli distribution if it have a binary values $X\varepsilon {0, 1}$ and its probability mass  function is 
$$
P(X=x)=
\left \{\begin{array}{cc}
0.78,& if x=1 \cr
...
which we write as $X_n \rightarrow^p\mu$
